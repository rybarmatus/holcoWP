...
if($result = wp_check_password($goshop_config['random_password'], $user->user_pass, $user->ID)){ 
    $default_password = true;
}
if($default_password){ ?>

    <div class="alert alert-danger"><?= _('Prosím zadajte si nové heslo.'); ?></div>
...
<form class="woocommerce-EditAccountForm edit-account" action="" method="post" <?php do_action( 'woocommerce_edit_account_form_tag' ); ?> >
    ...
    <?php if($default_password) { ?>
         ...
         <input type="hidden" value="<?= $goshop_config['random_password']; ?>" class="form-control" name="password_current" id="password_current" autocomplete="off" />
	...
	<p>
		<label for="password_1"><?php if($default_password) { echo _('Nové heslo').'&nbsp;<span class="required">*</span>'; } else { esc_html_e( 'New password (leave blank to leave unchanged)', 'woocommerce' );  }?></label>
	...
	<p>
		<label for="password_2"><?php esc_html_e( 'Confirm new password', 'woocommerce' ); ?><?php if($default_password) { echo '&nbsp;<span class="required">*</span>'; } ?></label>
	...
	<p>
		...
		<button type="submit" class="btn btn-success mt-1" name="save_account_details" value="<?php esc_attr_e( 'Save changes', 'woocommerce' ); ?>"><?php esc_html_e( 'Save changes', 'woocommerce' ); ?></button>
